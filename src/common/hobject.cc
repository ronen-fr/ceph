...
namespace {
static void escape_special_chars(const string& in, string* out)
{
...
string hobject_t::to_str() const
{
  ...
  string out = [=, this]() {
    if (snap == CEPH_NOSNAP) {
      return fmt::format(FMT_COMPILE("{:016X}.{:08X}.head."), poolid, revhash);
    } else if (snap == CEPH_SNAPDIR) {
      return fmt::format(
	  FMT_COMPILE("{:016X}.{:08X}.snapdir."), poolid, revhash);
    } else {
      return fmt::format(
