...
class TestScrubber : public ScrubBeListener, public SnapMapperAccessor {
 public:
  ...
  void set_snaps(const hobject_t& hoid, const ScrubGenerator::all_clones_snaps_t& clones_snaps)
  {
    for (const auto& [clone, snaps] : clones_snaps) {
      std::cout
        << fmt::format("{}: ({}) -> #{} {}", __func__, clone, snaps.size(), snaps)
        << std::endl;
      std::set<snapid_t> snaps_set(snaps.begin(), snaps.end());
  ...
  ScrubGenerator::RealObjsConfList& all_sets)
{
  ...
  for (int osd_num = 0; osd_num < pool_conf.size; ++osd_num) {
    ...
    for (auto& obj : all_sets[osd_num]->objs) {
      ...
      std::cout << fmt::format("{}: object: {}\n", __func__, obj.object_version.ghobj.hobj);
      /*auto objects_snapset = */ ScrubGenerator::add_object(smap,
                                                             obj,
                                                             osd_num);
      // test_scrubber->set_snaps(obj.real_versions[0].ghobj.hobj,
      //                          objects_snapset);
      //test_scrubber->set_snaps(obj.object_version.ghobj.hobj, {20,30,40});
    }
