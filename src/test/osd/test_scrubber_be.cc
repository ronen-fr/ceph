...
class TestScrubber : public ScrubBeListener, public SnapMapperAccessor {
 public:
  ...
  TestScrubber(spg_t spg, OSDMapRef osdmap, LoggerSinksSet* logger)
      : m_spg{spg}
      , m_logger{logger}
      , m_osdmap{osdmap}
  {}
  ...
  LoggerSinksSet* get_logger() const final { return m_logger; }
...
TEST_F(TestTScrubberBe_data_1, snapmapper_1)
{
  ...
  ASSERT_TRUE(fix_list[0].hoid == hobj_ms1_snp30_inpool);
  ASSERT_TRUE(fix_list[0].snaps == std::set<snapid_t>{0x30});
