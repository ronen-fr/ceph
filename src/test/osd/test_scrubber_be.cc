...
class TestScrubBackend : public ScrubBackend {
 public:
  ...
  {
    ceph::common::ConfigProxy conf{true};

    ceph::logging::Log* douter = new ceph::logging::Log(&conf->subsys);
    douter->set_log_stderr_prefix("+++++");
    ctx._log = douter;
  }
  mutable CephContext ctx{CEPH_ENTITY_TYPE_OSD, CODE_ENVIRONMENT_DAEMON};
...
class TestScrubber : public ScrubBeListener, public SnapMapperAccessor {
 public:
  ...
  TestScrubber(spg_t spg, OSDMapRef osdmap, LoggerSinkSet* logger)
      ...
      , m_osdmap{osdmap}
  {
    // replace the dout target with our own, thru the context
    ceph::common::ConfigProxy conf{true};

    ceph::logging::Log* douter = new ceph::logging::Log(&conf->subsys);
    douter->set_log_stderr_prefix("+++++");
    douter->set_log_file("/dev/null");
    ctx._log = douter;
  }
  ...
  CephContext* get_pg_cct() const final { return &ctx; /*g_ceph_context; */ }

  CephContext* get_osd_cct() const final { return &ctx; /*g_ceph_context;*/ }
  ...
  bool m_primary{true};
  spg_t m_spg;
  LoggerSinkSet* m_logger{nullptr};
  OSDMapRef m_osdmap;
  pg_info_t m_info;
...
pg_info_t TestTScrubberBe::setup_pg_in_map()
{
  ...
  info.stats.stats.sum.num_objects_misplaced = 0;
  info.stats.stats.sum.num_objects_unfound = 0;
  // info.stats.stats.sum.num_bytes_used = 0; // FIXME: find the right value
