...
/// \file testing the output of various dump/query commands
...
// a do-nothing FSM mock

class MockScrubFsm : public Scrub::ScrubFsmIf {
 public:
  void process_event(const boost::statechart::event_base& evt) override {}
  [[nodiscard]] bool is_primary_idle() const override { return true; }
  [[nodiscard]] bool is_reserving() const override { return false; }
  [[nodiscard]] bool is_accepting_updates() const override { return false; }
  void assert_not_in_session() const override {}
  ceph::timespan get_time_scrubbing() const override { return ceph::timespan{}; }
  std::optional<pg_scrubbing_status_t> get_reservation_status()
    const override
  {
    return std::nullopt;
  }
...
// a testable version of the PgScrubber

class TestPgScrubber : public PgScrubber {
 public:
  TestPgScrubber(PG* pg)
      : PgScrubber(pg)
  {
    // override the FSM with a mock
    m_fsm = std::make_unique<MockScrubFsm>();
  }

  // expose the scrubber's internal state for testing
};

// ///////////////////////////////////////////////////////////////////////////
// ///////////////////////////////////////////////////////////////////////////

// and the main test structure

class TestPgScrubberFormats : public ::testing::Test {
 protected:
  void SetUp() override;
  ...
  // test objects
  TestPgScrubber* m_pg_scrubber{nullptr};  // the object under test

  // helper functions for testing can be added here
};

void TestPgScrubberFormats::SetUp()
{
  // setup code for each test can go here
  // e.g. initializing objects, setting up mock data, etc.
