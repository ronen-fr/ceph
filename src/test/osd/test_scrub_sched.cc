...
std::vector<sjob_config_t> sjob_configs = {
...

  {
    spg_t{pg_t{5, 1}},
    true,
    utime_t{},	// RRR a specific time value
    1.0,
    std::nullopt,
    false,
    false,
    ScrubQueue::scrub_schedule_t{} /*ceph_clock_now(), utime_t{0}*/
...
/// validate the states of teh scrub-jobs (as set in the jobs themselves)
TEST_F(TestScrubSched, states)
{
  ...
  EXPECT_EQ(0, count_scrub_jobs_in_state(ScrubQueue::qu_state_t::not_registered));
  ...
  m_sched->remove_from_osd_queue(m_scrub_jobs[2].job);	// won't work
  ...
  EXPECT_EQ(1, count_scrub_jobs_in_state(ScrubQueue::qu_state_t::registered));
  EXPECT_EQ(2,
	    count_scrub_jobs_in_state(ScrubQueue::qu_state_t::unregistering));
  m_sched->rm_unregistered_jobs();
  EXPECT_EQ(2, count_scrub_jobs_in_state(ScrubQueue::qu_state_t::not_registered));
