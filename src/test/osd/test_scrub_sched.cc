...
class ScrubSchedTestWrapper : public ScrubQueue {
 public:
  ...
  // provide access to the private 'collect_ripe_jobs()'
...
class TestScrubSched : public ::testing::Test {
 ...
 protected:
  ...
  int count_scrub_jobs_in_state(ScrubQueue::qu_state_t state)
  {
...
static const auto epoch_2000 = 946684800;
...
/// basic test of scheduling simle jobs, validating their calculated schedule
...
{
  m_sched->set_time_for_testing(0);
  std::for_each(sjob_configs.begin(),
		sjob_configs.end(),
  ...
  std::cout << "State: # registered "
	    << count_scrub_jobs_in_state(ScrubQueue::qu_state_t::registered)
	    << std::endl;
  std::cout << "State: # unregistering: "
	    << count_scrub_jobs_in_state(ScrubQueue::qu_state_t::unregistering)
	    << std::endl;
  m_sched->rm_unregistered_jobs();
  EXPECT_EQ(0,
	    count_scrub_jobs_in_state(ScrubQueue::qu_state_t::not_registered));
  ...
  m_sched->remove_from_osd_queue(m_scrub_jobs[2].job);	// will have no effect
  ...
  EXPECT_EQ(2, count_scrub_jobs_in_state(ScrubQueue::qu_state_t::registered));
  EXPECT_EQ(2,
	    count_scrub_jobs_in_state(ScrubQueue::qu_state_t::unregistering));
...
TEST_F(TestScrubSched, ready_list)
{
  ...
  for (const auto& job : all_reg_jobs) {
    std::cout << fmt::format("ready_list: job: {}", *job)
              << std::endl;
  }
  ...
  std::cout << fmt::format("ready_list: time now {}", m_sched->time_now())
	    << std::endl;
  for (auto& job : ripe_jobs) {
    std::cout << fmt::format("job {} ({}): scheduled {}",
			     job->pgid,
