...
public:
  ...
  void shorten_mapping_key(snapid_t snap, const hobject_t &clone)
  {
    ...
    std::cout << fmt::format("shorten_mapping_key: {}/{} -> {}", clone, snap, k)
	      << std::endl;
    ...
    // replace the ke with its shortened version
    PausyAsyncMap::Transaction t;
  ...
  void modify_object_entry(snapid_t snap, const hobject_t &clone)
  {
    // calculate the relevant key
    std::string k = mapper->to_raw_key(snap, clone);
    std::cout << fmt::format("shorten_mapping_key: {}/{} -> {}", clone, snap, k)
...
{
  //std::cout << "TestTScrubberBe::TearDown()" << std::endl;
  direct->driver->stop();
  direct->mapper.reset();
  direct->driver.reset();
  //std::cout << "TestTScrubberBe::TearDown() done" << std::endl;
}
