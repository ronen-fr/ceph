...
seastar::future<Scrub::schedule_result_t> PG::sched_scrub()
{
  ...
  if (!is_primary() || !peering_state.is_active() || !peering_state.is_clean()) {
    return seastar::make_ready_future<Scrub::schedule_result_t>(
...
{
  logger().info("{}: need_auto? {} allow_deep_scrub? {} allow_scrub? {} has_deep_errors? {}",
                __func__, planned.need_auto, allow_deep_scrub, allow_scrub, has_deep_errors);
  ...
  if (ceph_clock_now() >= next_deepscrub_interval()) {
    logger().debug("{}: now ({}) >= time for deep ({})",
                 __func__, ceph_clock_now(), next_deepscrub_interval());
    return true;
  }

  if (has_deep_errors) {
    shard_services.clog->info() << fmt::format("osd.{}: pg {}: Deep scrub errors, upgrading scrub to deep-scrub",
  ...
  if (allow_scrub) {
    bool deep_coin_flip =
      (rand() % 100) < get_cct()->_conf->osd_deep_scrub_randomize_ratio * 100;
...

{
  ...
  if (allow_deep_scrub) {
	...
	planned.auto_repair = true;
        ...
        logger().info("{}: auto repair with scrubbing, rescrub if errors found", __func__);
	planned.deep_scrub_on_error = true;
      }
    }
  }

  logger().info("{}: updated flags: {} allow_regular_scrub: {}", __func__, planned, allow_regular_scrub);
...
std::optional<requested_scrub_t> PG::verify_scrub_mode() const
{
  ...
  bool prevented_by_recovery =
    shard_services->is_recovery_active() && !cct->_conf->osd_scrub_during_recovery &&
    (!get_cct()->_conf->osd_repair_during_recovery || !upd_flags.must_repair);
