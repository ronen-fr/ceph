...
class ReplicaReservations {
  ...
  int m_pending{-1};
  const pg_info_t& m_pg_info;           // wasn't in scrubber.h
 ...
 /**
   ...
   * longer relevant, and that the replicas had reset the reservations on their
   * side.
   */
  void discard_all();
...
class LocalReservation {
 ...
 public:
  explicit LocalReservation(crimson::osd::ShardServices* osds);
  ~LocalReservation();
...
struct fmt::formatter<scrub_flags_t> {
  ...
  auto format(const scrub_flags_t& flg, FormatContext& ctx)
  {
    return format_to(ctx.out(), "{}{}{}{}",
                     flg.auto_repair ? " AUTO_REPAIR" : "",
                     flg.check_repair ? " CHECK_REPAIR" : "",
                     flg.deep_scrub_on_error ? " DEEP_SCRUB_ON_ERROR" : "",
                     flg.required ? " REQ_SCRUB" : "");
  }
};

std::ostream& operator<<(std::ostream& out, const scrub_flags_t& sf);
...
class PgScrubber : public ScrubPgIF, public ScrubMachineListener {
 ...
 public:
  ...
  [[nodiscard]] bool are_callbacks_pending()
    const final  // used for an assert in PG.cc
  {
    return !m_callbacks.empty();
  }

  /// handle a message carrying a replica map
  // RRR todo void map_from_replica(OpRequestRef op) final;
  ...
  [[nodiscard]] bool is_primary()
    const final;  // RRR move { return m_pg->recovery_state.is_primary(); }
  ...
  eversion_t get_last_update_applied() const final;  // RRR move
                                                     //   {
  //     return m_pg->recovery_state.get_last_update_applied();
  //   }

  int pending_active_pushes()
    const final;  // RRR move { return m_pg->active_pushes; }
 ...
 //RRR// std::unique_ptr<Scrub::Store> m_store;
  ...
  CephContext* get_pg_cct() const { return nullptr; /*return m_pg->cct;*/ }
...
public:
  ...
  void scrub_echo(epoch_t epoch_queued);
