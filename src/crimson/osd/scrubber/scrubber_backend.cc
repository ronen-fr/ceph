...
ScrubberBE::ScrubberBE(PgScrubber& scrubber,
		       PGBackend& backend,
		       pg_shard_t i_am,
		       bool repair,
    ...
    , m_primary_map{primary_map}
{
  ...
  logger().debug("{}: created for {} ({}). Repair: {}", __func__, scrubber.m_pg_id, i_am, repair);
}

// ///////////////////////////////////////////////////////////////////////// //

void ScrubberBE::new_chunk()
{
  this_chunk.emplace();
  this_chunk->maps[m_pg_whoami] = m_primary_map;
}
...
void ScrubberBE::merge_to_master_set()
{
...
void ScrubberBE::decode_received_map(pg_shard_t from,
...
seastar::future<> ScrubberBE::scrub_compare_maps(
...
seastar::future<> ScrubberBE::update_authoritative(stringstream& ss)
{
  ...
  return compare_smaps(ss).then([this, &ss]() mutable {
    ...
    logger().debug(
      "scrubber: ScrubberBE::scrub_compare_maps(): from compare_smaps(): {}", ss.str());
...
seastar::future<> ScrubberBE::compare_smaps(ostream& errstream)
{
...
std::optional<ScrubberBE::AuthAndObjErrors> ScrubberBE::empty_auth_list(
  ...
  ostream& errstream)
{
  ...
  return std::optional<ScrubberBE::AuthAndObjErrors>{
    ScrubberBE::AuthAndObjErrors{std::move(auths), std::move(obj_errors)}};
}


// called from the refactored
// PGBackend::be_compare_scrubmaps()/PgScrubber::compare_smaps()
seastar::future<> ScrubberBE::compare_obj_in_maps(const hobject_t& ho, ostream& errstream)
{
  ...
  return seastar::do_with(
	...
	.safe_then(
	  ...
	  PGBackend::ll_read_errorator::all_same_way([]() {
	    ...
	    logger().debug("ScrubberBE::compare_obj_in_maps() in error handling");
	    return seastar::make_ready_future<>();
	  }));
    });
}
...
seastar::future<> ScrubberBE::inconsistents(
  const hobject_t& ho,
...
ScrubberBE::digest_fixing_t ScrubberBE::should_fix_digest(const hobject_t& ho, const ScrubMap::object& auth_object,
...
// seastar::future<ScrubberBE::AuthAndObjErrors>
ScrubberBE::AuthAndObjErrors ScrubberBE::match_in_shards(
  IterToSMap auth,
  const hobject_t& ho,
 ...
 * backwards.
 */
ScrubMap ScrubberBE::clean_meta_map(ScrubMap& cleaned, bool max_reached)
{
...
bool ScrubberBE::compare_obj_details(pg_shard_t auth_shard,
 ...
 */
void ScrubberBE::scrub_snapshot_metadata(ScrubMap& map,
...
void ScrubberBE::record_object_digests(int missing)
{
  ...
  for_each(this_chunk->missing_digest.cbegin(), this_chunk->missing_digest.cend(),
	   [&/*, this*/](const auto& kv) {
	     ...
	     logger().debug(
	       "ScrubberBE::record_object_digests(): recording digests for {} ({}, {})",
	     ...
	     //   logger().debug("ScrubberBE::record_object_digests(): updating scrub
	     //   digest ({})",
	     //       m_scrubber.num_digest_updates_pending);
