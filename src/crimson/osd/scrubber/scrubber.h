...
class ReplicaReservations {
  // using OrigSet = decltype(std::declval<crimson::osd::PG>().get_actingset());

  ::crimson::osd::PG* m_pg;
  const std::set<pg_shard_t> m_acting_set;
 ...
 public:
  ...
  ReplicaReservations(::crimson::osd::PG* pg, pg_shard_t whoami);
...
class ReservedByRemotePrimary {
 ...
 public:
  ReservedByRemotePrimary(::crimson::osd::PG* pg,
...
class MapsCollectionStatus {
 ...
 public:
  ...
  void mark_local_map_ready()
  {
    m_local_map_ready = true;
  }
  ...
  std::vector<pg_shard_t> get_awaited() const
  {
  ...
  friend std::ostream& operator<<(std::ostream& out, const MapsCollectionStatus& sf);
};
...
struct scrub_flags_t {
  ...
  friend std::ostream& operator<<(std::ostream& out, const scrub_flags_t& sf);
};
...
class PgScrubber : public ScrubPgIF, public ScrubMachineListener {
 ...
 public:
  explicit PgScrubber(::crimson::osd::PG* pg);

  friend ScrubBackend; // our backend object
  ...
  void queue_regular_scrub() final;
  ...
  void send_sched_replica(epoch_t epoch_queued) final;  // not in crimson
  ...
  void send_maps_compared(epoch_t epoch_queued) final; // crimson
   ...
   *  scrubbing
   */
  void handle_scrub_reserve_request(const MOSDScrubReserve& req, pg_shard_t from) final;
  ...
  ScrubQueue::sched_params_t determine_scrub_time(const requested_scrub_t& request_flags);

  //void unreg_next_scrub() final;

  void register_with_osd() final;

  void unregister_from_osd() final;
  ...
  void add_callback(Context* context) final
  {
  ...
  int pending_active_pushes() const final;
 ...
 public:
  ...
  friend std::ostream& operator<<(std::ostream& out, const PgScrubber& scrubber);

  static utime_t scrub_must_stamp()
  {
    return utime_t(1, 1);
  }

  ~PgScrubber();  // must be defined separately, in the .cc file
 ...
 private:
  ...
  void queue_local_trigger(::crimson::osd::ScrubEventFwd trigger,
 ...
 private:
   ...
   */
  //void scrub_finish_pglog();


  // low-level - to be moved out

  /**
   * Collect the maps from all shards into one collection.
   * Then - collect the objects from all those maps.
   *
   * @return the collected maps arranged by pg_shard_t, and a merged set of all objects in all scrub-maps
   */
  //std::tuple< std::map<pg_shard_t, ScrubMap*>, std::set<hobject_t>> merge_to_master_set();
  ...
  seastar::future<> repair_oinfo_oid(ScrubMap& smap);

  // common code used by build_primary_map_chunk() and build_replica_map_chunk():
  seastar::future<> build_scrub_map_chunk(ScrubMap& map,  // primary or replica?
					  ScrubMapBuilder& pos,
					  hobject_t start,
					  hobject_t end,
					  scrub_level_t shallow_or_deep);

  seastar::future<> scan_rollback_obs(const std::vector<ghobject_t>& rollback_obs);
 ...
 private:
  ...
  std::unique_ptr<Scrub::ScrubberBE> m_be;

  /// Cleaned std::map pending snap metadata scrub
  ScrubMap cleaned_meta_map; // for the replica

  seastar::future<> request_scrub_map(pg_shard_t replica,
				      eversion_t version,
				      hobject_t start,
				      hobject_t end,
				      bool deep,
  ...
  class preemption_data_t : public Scrub::preemption_t {
   public:
    ...
    [[nodiscard]] bool is_preemptable() const final
    {
    ...
    [[nodiscard]] bool was_preempted() const
    {
      return m_preempted;
    }

    [[nodiscard]] size_t chunk_divisor() const
    {
      return m_size_divisor;
    }
   ...
   private:
    ...
    bool are_preemptions_left() const
    {
      return m_left > 0;
    }
