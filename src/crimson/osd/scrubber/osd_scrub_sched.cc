...
bool ScrubQueue::scrub_time_permit(utime_t now) const
{
  ...
  bool day_permit = isbetween_modulo(local_conf()->osd_scrub_begin_week_day,
				     local_conf()->osd_scrub_end_week_day,
				     bdt.tm_wday);
  if (!day_permit) {
  ...
  bool time_permit = isbetween_modulo(local_conf()->osd_scrub_begin_hour,
				      local_conf()->osd_scrub_end_hour,
				      bdt.tm_hour);
  dout(20) << "should run between " << local_conf()->osd_scrub_begin_hour
	   << " - " << local_conf()->osd_scrub_end_hour << " now ("
