...
ScrubEvent::ScrubEvent(ShardServices& shard_services,
                       const spg_t& pgid,
                       epoch_t epoch,
                       std::chrono::milliseconds delay)
    : shard_services{shard_services}  //, ctx{ceph_release_t::octopus}
                                      //, from{from}
    , pgid{pgid}
    , epoch_queued{epoch}
    , delay{delay}
    , dbg_desc{"<ScrubEventCtor2>"}
{
  if (delay.count() > 0) {
    logger().debug("ScrubEvent: 2nd ctor {:p} {} delay:{}", (void*)this,
                   dbg_desc, delay);
  }
}
...
ScrubEvent::ScrubEvent(ShardServices& shard_services,
                       const spg_t& pgid,
                       epoch_t epoch)
    : ScrubEvent(shard_services, pgid, epoch, 0ms)
{
  logger().debug("ScrubEvent: 1st ctor {:p} {}", (void*)this, dbg_desc);
}
