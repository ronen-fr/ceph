...
{
  out << "device_shard_info("
      << "size=0x" << std::hex << si.size
      << ", segments=" << std::dec << si.segments
      << ", first_segment_offset=0x" << std::hex << si.first_segment_offset
      << ", tracker_offset=0x" << si.tracker_offset
      << ", start_offset=0x" << si.start_offset << std::dec
      << ")";
  return out;
}
...
{
  out << "device_superblock("
      << "version=" << (unsigned)sb.version
      << ", shard_num=" << sb.shard_num
      << ", segment_size=0x" << std::hex << sb.segment_size
      << ", block_size=0x" << sb.block_size << std::dec
      << ", config=" << sb.config
      << ", total_size=0x" << std::hex << sb.total_size
      << ", journal_size=0x" << sb.journal_size
      << ", segment_capacity=0x" << sb.segment_capacity
      << ", zones_per_segment=" << std::dec << sb.zones_per_segment
      << ", zone_size=0x" << std::hex << sb.zone_size
      << ", zone_capacity=0x" << sb.zone_capacity << std::dec
      << ", crc=" << sb.crc
      << ", feature=0x" << std::hex << sb.feature << std::dec
      << ", nvme_block_size=" << sb.nvme_block_size
      << ", shards:[";
  for (const auto& si : sb.shard_infos) {
    out << si << ",";
  }
  return out << "])";
}
