                                   ...
                                   [[maybe_unused]] bufferlist& out) const final
    {
      ...
      if (!cmd_getval<std::string>(nullptr, cmdmap, "var", var) ||
        ...
        return seastar::futurize_apply([this, valstr, var] {
          ...
          .handle_exception([](std::exception_ptr eptr) { return; });
      }
    }
  };
...
//  some PIMPL details:
//
ContextAdmin::ContextAdmin(CephContext* cct, crimson::common::ConfigProxy& conf)
    : m_imp{ std::make_unique<ContextAdminImp>(cct, conf) }
{}

seastar::future<> ContextAdmin::register_admin_commands(
