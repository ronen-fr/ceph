...
/*
  * note: ModeCollector inherits from xx, so that 
...
namespace std {
  ...
  struct hash<eversion_t> {
    size_t operator()(const eversion_t& ev) const noexcept {
      ...
      return (size_t)ev.epoch ^ ((size_t)ev.version << 8 | (size_t)ev.version >> (sizeof(size_t)*8 - 8));
    }
  };
}
#endif
