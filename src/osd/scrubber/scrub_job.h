...
class ScrubJob final : public RefCountedObject {
 public:
  ...
  /**
   *  'reset_notbefore' is used to reset the 'not_before' jo attribute to the
   *  updated 'scheduled_at' time. This is used whenever the scrub-job schedule
   *  is updated, and the update is not a result of a scrub attempt failure.
   */
  void update_schedule(const scrub_schedule_t& adjusted, bool reset_notbefore);
