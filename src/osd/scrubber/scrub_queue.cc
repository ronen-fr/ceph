...
class ScrubQueueImp : public ScrubQueueImp_IF {
 ...
 public:
  ScrubQueueImp(
      Scrub::ScrubQueueOps& parent_queue)  // : RRR parent_queue(parent_queue)
  {}

  void enqueue_entry(const SchedEntry& entry) override;

  bool remove_entry(spg_t pgid, scrub_level_t s_or_d) override;

  ScrubQueueStats get_stats(utime_t scrub_clock_now) override;
...
bool ScrubQueue::enqueue_targets(
    spg_t pgid,
    ...
    const SchedEntry& deep)
{
  ...
  // RRR document how this can happen
    ...
    bool is_recovery_active)
{
  ...
  if (g_conf()->subsys.should_gather<ceph_subsys_osd, 20>()) {
    auto select_ready = [](const SchedEntry&, bool is_eligible) -> bool { return is_eligible; };

    dout(20) << fmt::format("{}: the scrub targets queue:", __func__) << dendl;
    auto all_jobs = m_queue_impl->get_entries(select_ready);
    for (const auto& sj : all_jobs) {
      dout(20) << fmt::format(" scrub-queue job: {}", sj) << dendl;
    }
  }
  if (g_conf()->subsys.should_gather<ceph_subsys_osd, 20>()) { // RRR make that 30
    auto select_ready = [](const SchedEntry&, bool is_eligible) -> bool { return is_eligible; };
  ...
  auto env_restrictions =
      restrictions_on_scrubbing(conf(), is_recovery_active, scrub_tick_time);
  if (!env_restrictions) {
    return;
  }
...
dout(10) << fmt::format(
		    "{}: in queue: {} ready, {} future, {} total", __func__,
                        queue_stats.num_ready, queue_stats.num_total - queue_stats.num_ready,
                        queue_stats.num_total) << dendl;
...
void ScrubQueue::scrub_next_in_queue(utime_t loop_id)
{
  ...
  if (!m_initiation_loop) {
    dout(20) << fmt::format(
		    "{}: no active scrub-loop ({} suggested)", __func__,
		    loop_id)
	     << dendl;
    return;
  }
...
void ScrubQueue::on_config_times_change()
{
  ...
  for (const auto& p : to_notify) {
    ...
    dout(15) << fmt::format("{}: updating scrub schedule on {}", __func__,
	(locked_pg->pg())->get_pgid()) << dendl;
...
working - but better use the mapped interface
bool ScrubQueueImp::remove_entry(spg_t pgid, scrub_level_t s_or_d)
{
  auto same_id = [pgid, s_or_d](const SchedEntry& e) {
    return e.pgid == pgid && e.level == s_or_d;
  };
  return to_scrub.remove_if<decltype(same_id)>(std::move(same_id));
}
#endif

bool ScrubQueueImp::remove_entry(spg_t pgid, scrub_level_t s_or_d)
{
  auto same_lvl = [s_or_d](const SchedEntry& e) {
    return e.level == s_or_d;
  };
  return to_scrub.remove_if_by_class<spg_t, decltype(same_lvl)>(pgid, std::move(same_lvl), 1);
}
...
std::vector<SchedEntry> ScrubQueueImp::get_entries(EntryPred pred) const
{
  ...
  auto by_pred =
      [pred](acc_t&& acc, const SchedEntry& se, bool is_eligible) mutable -> acc_t {
	//if (pred(se, is_eligible)) {
	  acc.push_back(se);
	//}
	return std::move(acc);
