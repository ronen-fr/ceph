...
ScrubQueue::ScrubQueue(CephContext* cct)
    : cct{cct}
    , osd_service{osds}
...
std::optional<ScrubTargetId> ScrubQueue::pg_to_scrub(bool active_recovery)
{
  utime_t scrub_tick_time = scrub_clock_now();
  dout(10) << fmt::format(
		  "{}: time now:{}, is_recovery_active:{}", __func__, scrub_tick_time,
		  is_recovery_active)
	   << dendl;


  // are we already in the process of a scrub initiation? do not start another
  // until replica reservation is done.

  // RRR

  // what is the state of the queue?
  // Note: 'Reef' version of the queue for now
