...
struct ProcessingReservation;  ///< a reservation request was received from
 ...
 *
 *      * ProcessingReservation - a reservation request was received from
 ...
 *   We transition to ProcessingReservation, and wait for the Reserver's
 *   response.
...
struct ReplicaUnreserved : sc::state<ReplicaUnreserved, ReplicaIdle>,
  ...
  sc::result react(const ReplicaReserveReq& eq);
 ...
 *
 * Possible events:
 * - 'go ahead' from the asynch reserver. We send a GRANT message to the
 ...
 */
struct ProcessingReservation : sc::state<ProcessingReservation, ReplicaIdle>,
			       NamedSimply {
  explicit ProcessingReservation(my_context ctx);
...
struct ReplicaActiveOp
  ...
  //sc::transition<FullReset, sc::shallow_history<ReplicaIdle>> >; // RRR needed, isn't it???
