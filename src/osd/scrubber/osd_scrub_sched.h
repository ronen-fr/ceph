...
class ScrubQueue {
 public:
  ...
  struct ScrubJob final : public RefCountedObject {
    ...
    /**
     * m_updated is a temporary flag, used to create a barrier after
     * m_sched_time and m_deadline (or any other job entry) were modified by
     * different task.
     * m_updated also signals the need to move a job back from the penalized
    ...
    /*
     * as the atomic 'm_in_q' appears in many log prints, accessing it for
   ...
   *   This happens if our stats are unknown. The results is similar to the
   *   previous scenario.
   *
 ...
 private:
  ...
  struct time_indirect_t {
    bool operator()(const ScrubJobRef& lhs, const ScrubJobRef& rhs) const
    {
   ...
   */
  TimeAndDeadline adjust_target_time(
