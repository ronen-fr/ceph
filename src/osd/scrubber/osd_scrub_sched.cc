  ...
  const Scrub::scrub_schedule_t& adjusted)
{
  ...
  // the (atomic) flag will only be cleared by select_pg_and_scrub() after // RRR fix comment
...
// Note: 'restrictions' is unsed in this version (step 1 of the refactoring)
...
// RRR handle the "reached the penalized - so all should be forgiven" case
