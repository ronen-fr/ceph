				   ...
				   const ScrubQueue::sched_params_t& suggested)
{
  ...
  switch (scrub_job->state) {
    ...
    case qu_state_t::not_registered:
      ...
      {
	std::unique_lock lck(jobs_lock, 2'000ms);
	ceph_assert(lck.owns_lock());
  ...
  dout(15) << "pg(" << scrub_job->pgid
	   << ") inserted: " << scrub_job->sched_time << dendl;
}
  ...
  for (auto job = to_scrub.begin(); job != to_scrub.end();) {
    ...
    if ((*job)->resources_failure) {
      ...
      dout(15) << "moving " << sjob->pgid
	       << " state: " << ScrubQueue::qu_state_text(sjob->state)
	       << dendl;
      ...
      sjob->updated = false;	// as otherwise will immediately be pardoned
 ...
 */
std::string_view ScrubQueue::attempt_res_text(Scrub::attempt_t v)
{
  switch (v) {
    case Scrub::attempt_t::scrub_initiated: return "scrubbing"sv;
    case Scrub::attempt_t::none_ready: return "no ready job"sv;
    case Scrub::attempt_t::no_local_resources: return "local resources shortage"sv;
    case Scrub::attempt_t::already_started: return "already started"sv;
    case Scrub::attempt_t::no_such_pg: return "pg not found"sv;
    case Scrub::attempt_t::bad_pg_state: return "prevented by pg state"sv;
...
{
  ScrubQueue::TimeAndDeadline sched_n_dead{times.proposed_time,
					   times.proposed_time};
...
bool ScrubQueue::scrub_time_permit(utime_t now) const
{
  ...
  dout(20) << "should run between " << cct->_conf->osd_scrub_begin_hour
	   << " - " << cct->_conf->osd_scrub_end_hour << " now ("
...
{
  dout(20) << ": " << scrubs_local << " -> " << (scrubs_local - 1)
	   << " (max " << cct->_conf->osd_max_scrubs << ", remote "
	   << scrubs_remote << ")" << dendl;
