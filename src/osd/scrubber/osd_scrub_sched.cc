    ...
    utime_t now_is)
{
  ...
  merge_delay_requeue(std::move(aborted_target), issue, delay, false, now_is);
}
    ...
    utime_t now_is)
{
  ...
  dout(10) << fmt::format(
		  ...
		  aborted_target, nxt_target, delay_both)
	   << dendl;
 ...
 *   both targets.
 */
void ScrubJob::on_reservation_failure(SchedTarget&& aborted_target, utime_t now_is)
{
  ++consec_aborts;
  const seconds delay = seconds(
      cct->_conf.get_val<int64_t>("osd_scrub_retry_busy_replicas"));
  merge_delay_requeue(std::move(aborted_target), delay_cause_t::replicas, delay, true, now_is);
}
