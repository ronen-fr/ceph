  ...
  const ScrubQueue::scrub_schedule_t& adjusted)
{
  ...
  dout(10) << __func__ << ": pg[" << pgid
	   << "] adjusted: " << schedule.scheduled_at << "  "
  ...
  const ScrubQueue::sched_params_t& suggested)
{
  ...
  dout(20) << fmt::format(
		"pg[{}] state at entry: <{:14.14}>", scrub_job->pgid,
		state_at_entry)
	   << dendl;
  ...
  dout(10) << fmt::format(
		"pg[{}] sched-state changed from <{:14.14}> to <{:14.14}>",
  ...
  const sched_params_t& times) const
{
  ...
  if (times.is_must == ScrubQueue::must_scrub_t::not_mandatory) {
    ...
    dout(20)
      << fmt::format(
	   "not-must. Was:{:s}; {{min:{}/{} max:{}/{} ratio:{}}} Adjusted:{:s} ({:s})",
	   times.proposed_time, times.min_interval,
	   fmt::group_digits(conf()->osd_scrub_min_interval), times.max_interval,
	   conf()->osd_scrub_max_interval,
