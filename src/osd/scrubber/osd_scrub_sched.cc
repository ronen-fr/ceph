...
/// @brief  \todo time the delay to a period which is based on the wait for
...
void SchedTarget::on_local_resources()
{
  ...
  push_nb_out(/* RRR conf */ 3s, delay_cause_t::local_resources);
}
...
void SchedTarget::dump(ceph::Formatter* f) const
{
  ...
  f->dump_bool("forced", target == PgScrubber::scrub_must_stamp());
...
#ifdef NOTYET
  dout(20) << fmt::format(
		"{}: pg: [{}] flags: {} conf: {}", __func__, pgid,
		request_flags, config)
	   << dendl;
#endif
  ...
  const requested_scrub_t& request_flags)
{
  ...
  // must not reassign 'trgt'. Thus - using 'if' initialization

  // the job's shallow target
  if (auto& trgt = get_current_trgt(scrub_level_t::shallow);
  ...
  if (auto& trgt = get_current_trgt(scrub_level_t::deep);
      !trgt.scrubbing && to_change_on_conf(trgt.urgency)) {
    if (trgt.urgency == urgency_t::penalized) {
      trgt.urgency = urgency_t::periodic_regular;
    }
...
void ScrubQueue::on_config_times_change()
{
  ...
  for (const auto& [job, lvl] : all_jobs) {
    ...
    dout(20) << fmt::format("examine {} ({})", job->pgid, trgt)
	     << dendl;
    ...
    if (!pg->is_primary()) {
      dout(20) << fmt::format("{} is not primary", job->pgid)
               << dendl;
      continue;
    }
  ...
  dout(10) << fmt::format(
		"{} planned scrubs rescheduled", modified_cnt)
	   << dendl;
}
...
void ScrubJob::dump(ceph::Formatter* f) const
{
  ...
  f->dump_stream("deadline")
    << closest_target.get().deadline.value_or(utime_t{});
  f->dump_bool("forced", get_sched_time() == PgScrubber::scrub_must_stamp());
  f->close_section();
}
