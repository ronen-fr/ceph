...
/*
Difference from initial_shallow_target():
...
// RRR set_initial_targets() is now identical to at_scrub_completion(),
// and almost identical to on_periods_change(). Consider merging them.
    ...
    utime_t time_now)
{
  ...
  // the job's shallow target
  if (auto& trgt = get_current_trgt(scrub_level_t::shallow);
      !trgt.scrubbing && trgt.is_periodic()) {
    if (trgt.urgency == urgency_t::penalized) {
      trgt.urgency = urgency_t::periodic_regular;
    }
