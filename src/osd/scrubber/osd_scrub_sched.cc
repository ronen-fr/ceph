    ...
    utime_t time_now)
{
  ...
  const auto eligible_filtr = [time_now, rst = restrictions](
    ...
    // look for jobs that for at least one of their pair of targets,
    // the target's n.b. is in the past, and is not blocked by restrictions
      ...
      return t.is_ripe(time_now) &&
	     (t.is_high_priority() ||
	      (!rst.high_priority_only &&
	       (!rst.only_deadlined ||
		(!t.sched_info.schedule.deadline.is_zero() &&
		 t.sched_info.schedule.deadline <= time_now))));
    };
