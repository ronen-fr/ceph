...
#undef dout_prefix
#define dout_prefix *_dout << "osd." << m_whoami << "  "
...
{
  dout(10) << "removing pg(" << scrub_job->pgid << ") from OSD scrub queue"
	   << dendl;

  qu_state_t expected_state{qu_state_t::registered};
  auto ret = scrub_job->m_state.compare_exchange_strong(
...
{
  qu_state_t state_at_entry = scrub_job->m_state.load();
  ...
  switch (scrub_job->m_state) {
	...
	if (state_at_entry != scrub_job->m_state) {
	  l.unlock();
  ...
  dout(10) << "pg(" << scrub_job->pgid << ") sched-state changed from "
	   ...
	   << qu_state_text(scrub_job->m_state) << dendl;
  dout(15) << "pg(" << scrub_job->pgid
	   << ") inserted: " << scrub_job->m_sched_time << dendl;
}
			    ...
			    const ScrubQueue::sched_params_t& suggested)
{
  ...
  dout(10) << " pg(" << scrub_job->pgid
	   << ") adjusted: " << scrub_job->m_sched_time << "  "
  ...
  for (auto jb = to_scrub.begin(); jb != to_scrub.end();) {
    ...
    if ((*jb)->m_resources_failure) {
      ...
      dout(15) << "moving " << sjob->pgid
	       << " state: " << ScrubQueue::qu_state_text(sjob->m_state)
	       << dendl;
							  ...
							  utime_t time_now)
{
  ...
  std::copy_if(group.begin(), group.end(), std::back_inserter(ripes),
		 ...
		 return jobref->m_sched_time <= time_now;
	       });
					       ...
					       utime_t now_is)
{
  ...
  for (auto& candidate : group) {
    ...
    switch (m_osds.initiate_a_scrub(candidate->pgid,
...
{
  ScrubQueue::TimeAndDeadline sched_n_dead{times.suggested_stamp,
					   times.suggested_stamp};
...
bool ScrubQueue::scrub_load_below_threshold() const
{
  ...
  dout(20) << "loadavg " << loadavgs[0] << " >= max "
	   << m_cct->_conf->osd_scrub_load_threshold
...
bool ScrubQueue::scrub_time_permit(utime_t now) const
{
  ...
  bool day_permit =
    isbetween_modulo(m_cct->_conf->osd_scrub_begin_week_day,
  ...
  bool time_permit =
    isbetween_modulo(m_cct->_conf->osd_scrub_begin_hour,
		     m_cct->_conf->osd_scrub_end_hour, bdt.tm_hour);
  dout(20) << "should run between " << m_cct->_conf->osd_scrub_begin_hour
	   << " - " << m_cct->_conf->osd_scrub_end_hour << " now ("
	   << bdt.tm_hour << ") = " << (time_permit ? "yes" : "no") << dendl;
  return time_permit;
}
...
{
  dout(20) << ": " << m_scrubs_local << " -> " << (m_scrubs_local - 1)
	   << " (max " << m_cct->_conf->osd_max_scrubs << ", remote "
	   << m_scrubs_remote << ")" << dendl;
...
bool ScrubQueue::inc_scrubs_remote()
{
  ...
  if (m_scrubs_local + m_scrubs_remote < m_cct->_conf->osd_max_scrubs) {
    dout(20) << ": " << m_scrubs_remote << " -> " << (m_scrubs_remote + 1)
	     << " (max " << m_cct->_conf->osd_max_scrubs << ", local "
	     << m_scrubs_local << ")" << dendl;
    ++m_scrubs_remote;
    return true;
  }
