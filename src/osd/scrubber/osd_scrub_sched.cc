    ...
    utime_t time_now)
{
  ...
  if (pg_info.stats.stats_invalid && config.mandatory_on_invalid) {
	...
	sched_info.urgency = urgency_t::overdue;
      ...
      sched_info.deadline = SchedEntry::eternity;
    }
  }
    ...
    utime_t time_now)
{
  ...
  // deep scrub if not stats are available (only a shallow one)
...
{
  auto& moved_trgt =
      (s_or_d == scrub_level_t::shallow) ? shallow_target : deep_target;
    ...
    utime_t scrub_clock_now)
{
  ...
  bool log_as_updated{false};
  const bool upgrade_to_deep =
      (in_q_count == 0) && shallow_target.is_periodic() &&
      deep_target.is_periodic() && (rand() % 100) < aconf.deep_randomize_pcnt;
    ...
    utime_t scrub_clock_now)
{
  ...
  if (trgt.in_queue && trgt.is_required()) {
    dout(10) << fmt::format(
		    "{}: higher urgency scrub already in queue", __func__)
	     << dendl;
    return;
  }
    ...
    utime_t now_is)
{
  ...
  ceph_assert(nxt_target.is_viable());	// RRR if set to off as not primary?
    ...
    SchedTarget&& aborted_target)
{
  ...
  if (penalty_period.count() > 0 && shallow_target.is_periodic() &&
    ...
    penalized = true;
    penalty_timeout = now_is + utime_t{penalty_period};
    trgts_demoted = true;
  }
  scrub_queue.queue_entries(
      pgid, shallow_target.queued_element(), deep_target.queued_element());
