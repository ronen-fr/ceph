...
void PgScrubber::on_pg_activate()
{
  ...
  if (!m_scrub_job) {
    ...
    dout(2) << fmt::format(
		   "{}: {}.Reg-state:{:.7}. No scrub-job", __func__,
		   (is_primary() ? "Primary" : "Replica/other"),
		   registration_state())
	    << dendl;
    return;
  ...
  dout(10) << fmt::format(
		  "{} (from {}): {}. <{:.5}> --> <{:.5}> ({})", __func__,
		  "-XX-", (is_primary() ? "Primary" : "Replica/other"), pre_reg,
...
{
  // RRR lock? the relevant PG itself is locked, so no need to lock the queue?
