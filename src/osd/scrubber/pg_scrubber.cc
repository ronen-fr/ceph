...
pg_scrubbing_status_t PgScrubber::get_schedule() const
{
  ...
  if (m_active) {
    // update the scrubbing duration

    // RRR change to calculate in chrono spans
    ...
    // RRR note: make this into a separate function, replacing
    // set_scrub_duration()
    m_pg->modify_pg_stats(
      [&](pg_stat_t& s, epoch_t lec) { s.scrub_duration = duration; });
