...
void PgScrubber::scrub_finish()
{
  ...
  auto applicable_conf = populate_config_params();
  auto suggested = determine_scrub_time(m_pg->get_pgpool().info.opts);
  m_scrub_job->at_scrub_completion(suggested, applicable_conf, ceph_clock_now());
  m_osds->get_scrub_services().enqueue_target(*m_scrub_job);
...
Scrub::schedule_result_t PgScrubber::start_scrub_session(
    ...
    requested_scrub_t requested_flags)
{
    ...
    const requested_scrub_t& planned) const
{
  dout(10) << fmt::format(
		  ...
		  __func__, planned.need_auto, pg_cond.allow_shallow,
		  pg_cond.allow_deep, pg_cond.has_deep_errors,
        ...
        const requested_scrub_t& planned) const
{
  ...
  } else if (upd_flags.time_for_deep && pg_cond.allow_deep) { // already checked? RRR
    upd_flags.calculated_to_deep = true;
  } else {
    ...
    Scrub::ScrubPGPreconds pg_cond) const
{
  ...
  if (m_scrub_job->is_high_priority()) {
    ...
    dout(10) << fmt::format(
	     ...
	     << dendl;
    // RRR should we clear need_auto?
    return validate_initiated_scrub(pg_cond, time_for_deep, m_planned_scrub);
  }


  // --------  a periodic scrub
  dout(10) << fmt::format(
		  "{}: periodic target:{} pg:{}", __func__, *m_scrub_job, pg_cond)
	   << dendl;
