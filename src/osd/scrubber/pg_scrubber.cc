...
{
  dout(15) << __func__ << " was registered: " << is_scrub_registered() << dendl;
...
{
  return m_scrub_job && m_scrub_job->m_in_queue;
}
...
PgScrubber::determine_scrub_time(const requested_scrub_t& request_flags)
{
  ...
  if (request_flags.must_scrub || request_flags.need_auto) {
    ...
    res.suggested_stamp = m_pg->info.history.last_scrub_stamp;
    res.min_interval =
...
void PgScrubber::add_delayed_scheduling()
{
  ...
  if (m_needs_sleep) {
    double scrub_sleep = 1000.0 * m_osds->get_scrub_services().scrub_sleep_time(m_flags.required);
    ...
    , preemption_data{pg}
{
  ...
  m_scrub_job = ceph::make_ref<ScrubQueue::ScrubJob>(m_pg->cct, m_pg->pg_id, m_osds->get_nodeid());
}
...
{
  m_scrub_job->m_resources_failure = true;
