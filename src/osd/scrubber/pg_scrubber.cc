...
int size_from_conf(bool is_deep,
                   ceph::common::ConfigProxy& conf,
                   std::string deep_opt,
                   std::string shallow_opt)
{
  if (!is_deep) {
    auto sz = conf.get_val<int64_t>(shallow_opt);
    if (sz != 0) {
      // assuming '0' means no distinction was yet configured between deep and
      // shallow scrubbing
...
bool PgScrubber::select_range()
{
  ...
  dout(20)
    << fmt::format(
         ...
         __func__,
         (m_is_deep ? "D" : "S"),
         m_pg->get_cct()->_conf.get_val<int64_t>("osd_scrub_chunk_min"),
         m_pg->get_cct()->_conf.get_val<int64_t>("osd_shallow_scrub_chunk_min"),
         m_pg->get_cct()->_conf.get_val<int64_t>("osd_scrub_chunk_max"),
         m_pg->get_cct()->_conf.get_val<int64_t>("osd_shallow_scrub_chunk_max"))
    << dendl;

  const int min_from_conf = size_from_conf(m_is_deep,
                                           m_pg->get_cct()->_conf,
                                           "osd_scrub_chunk_min",
                                           "osd_shallow_scrub_chunk_min");
  const int max_from_conf = size_from_conf(m_is_deep,
                                           m_pg->get_cct()->_conf,
                                           "osd_scrub_chunk_max",
                                           "osd_shallow_scrub_chunk_max");

  const int min_chunk_sz =
    std::max(3, min_from_conf / preemption_data.chunk_divisor());

  const int max_chunk_sz =
    std::max(min_chunk_sz, max_from_conf / preemption_data.chunk_divisor());

  dout(10) << fmt::format("{}: Min: {} Max: {} Div: {}",
                          __func__,
                          min_chunk_sz,
                          max_chunk_sz,
