...
Scrub::CounterSetsInUse PgScrubber::get_counters_set() const
{
  // determine the labeled set to use:
  auto labeled_set = m_osds->get_scrub_services().get_perf_counters(
      (m_pg->pool.info.is_replicated() ? pg_pool_t::TYPE_REPLICATED
				       : pg_pool_t::TYPE_ERASURE),
      (m_is_deep ? scrub_level_t::deep : scrub_level_t::shallow));

  return CounterSetsInUse{
      .labeled_set = labeled_set,
      .counters = m_pg->pool.info.is_replicated() ? &io_counters_replicated
						  : &io_counters_ec};
}
