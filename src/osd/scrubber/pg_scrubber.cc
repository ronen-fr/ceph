...
void PgScrubber::set_scrub_blocked(utime_t since)
{
  ...
  m_osds->get_scrub_services().mark_pg_blockage(m_pg_id);
...
{
  ceph_assert(m_scrub_job->blocked); // should only be called when blocked
  m_osds->get_scrub_services().clear_pg_blockage(m_pg_id);
