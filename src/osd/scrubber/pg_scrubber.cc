...
void PgScrubber::on_init()
{
  ...
  //if (m_schedloop_step) {
    dout(5) << fmt::format("{}: scrub-initiation loop wasn't empty ({})",
                           __func__, *m_schedloop_step)
            << dendl;
    m_schedloop_step->conclude_candidates_selection();
...
void PgScrubber::on_repl_reservation_failure()
{
  ...
  m_scrub_job->on_reservation_failure(std::move(*m_active_target), ceph_clock_now());
  m_active_target.reset();
...
{
  dout(5) << __func__ << " backend error" << dendl;
...
{
  // No lock here. The relevant PG itself is locked (and this must be guaranteed, even if
  // the queue targets manipulation is performed directly by the OSD).
  ...
  //ssize_t all_enabled_cnt = 0; // also counting the optional m_active_target
  if (m_scrub_job) {
