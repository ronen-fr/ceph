 ...
 * As it happens - we do not care about the exact error value. Thus,
 * schedule_result_t is now reduced to a simple 'scrub_initiated' /
    ...
    const ScrubPreconds& preconds)
{
  ...
  // mark our target as not-in-queue. If any error is encountered - it
  // must be requeued!
  trgt.in_queue = false;
...
{
  using schedule_result_t = Scrub::schedule_result_t;
  if (trgt.sched_info.urgency >= urgency_t::operator_requested) {
    // 'initiated' scrubs
...
void PgScrubber::on_repl_reservation_failure()
{
  ...
  if (m_scrub_job->on_reservation_failure(
    ...
    m_depenalize_timer = std::make_unique<depenelize_timer_t>(
  ...
  if (m_scrub_job->on_reservation_failure(
    ...
    spg_t pgid = m_pg->get_pgid();
    Context* cp = new LambdaContext([scrbr = this, osds = m_osds,
				     pgid]([[maybe_unused]] int r) {
	...
	return;
      ...
      lgeneric_dout(scrbr->get_pg_cct(), 7)
	  << fmt::format("on_repl_reservation_failure: unpenalizing {}", pgid)
	  << dendl;
...
PgScrubber::depenelize_timer_t::depenelize_timer_t(
    OSDService* osds,
    PgScrubber* scrbr,
    utime_t timeout)
    : m_osds{osds}
    , m_scrbr{*scrbr}
{
  m_depenalize_cb = new LambdaContext(
	...
	// send an event to the scrubber to handle the timeout
	lgeneric_dout(g_ceph_context, 7)
	    << "depenelize_timer_t: callback activated" << dendl;
	...
	lgeneric_dout(g_ceph_context, 7)
	    << fmt::format("depenelize_timer_t: un-penalizing {}", pgid)
	    << dendl;
  ...
  std::lock_guard l(m_osds->sleep_lock);
  // add_event_after() deletes the callback if fails to add (and returns
  // nullptr)
  m_depenalize_cb = m_osds->sleep_timer.add_event_after(timeout, m_depenalize_cb);
}
    ...
    const Scrub::ScrubPgPreconds& pg_cond)
{
  ...
  m_flags.check_repair = trgt.sched_info.urgency == urgency_t::after_repair;
  ...
  // by the environment
  if (!trgt.is_deep() && pg_cond.can_autorepair && trgt.is_periodic()) {
