...
dout(19) << fmt::format("{}: for_meta_scrub: {}", __func__, for_meta_scrub) << dendl;
...
void ScrubBackend::scrub_snapshot_metadata(ScrubMap& map)
{
  ...
  vector<snapid_t>::reverse_iterator curclone;	// Defined only if snapset
						// initialized
  int missing = 0;
  ...
  SnapMapperAccessor& snaps_getter)
{
  ...
  for (auto i = smap.objects.rbegin(); i != smap.objects.rend(); ++i) {
	...
	continue;
      ...
      head = hoid.get_head(); // RRR understand why needed, i.e. why not just 'head = hoid'
      continue;
    }
	...
	derr << __func__ << " no head for " << hoid << " (have " << head << ")"
	     << dendl;
	dout(1) << __func__ << " no head for " << hoid << " (have " << head
		<< ")" << dendl;
	continue;
      }
