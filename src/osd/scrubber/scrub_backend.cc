  ...
  SnapMapReaderI& snaps_getter)
{
  ...
  for (const auto& ho : this_chunk->authoritative_set) {
    ...
    // evaluate the shards as a possible source of truth
    evaluate_object_shards(ho);
  }
...
void ScrubBackend::evaluate_object_shards(const hobject_t& ho)
{
  dout(10) << fmt::format("{}: object:{}", __func__, ho) << dendl;
  if (m_acting_but_me.empty()) {
    return;
  }
                                                  ...
                                                  stringstream& errstream)
{
  ...
  for (auto& l : shards) {
    ...
    ret_auth.shard_map[l] = shard_ret.shard_info;
                                                  ...
                                                  shard_info_map_t& shard_map_1)
{
  ...
  const auto& shard_smap = this_chunk->received_maps[srd];

  // const auto& j_shard = j->first;
  // const auto& j_smap = j->second;
  auto i = shard_smap.objects.find(obj);
  if (i == shard_smap.objects.end()) {
    // 'ret' defaults to 'not found'
    return ret;
  }
  const auto& smap_obj = i->second;

  //auto& shard_info = shard_map_1[srd];
  auto& shard_info = ret.shard_info;
  if (srd == m_pg_whoami) {
    shard_info.primary = true;
  }

  ret.possible_auth = usable_t::not_usable; // until proven otherwise
  ...
  if (obj.is_head() && !m_pg.get_is_nonprimary_shard(srd)) {
  ...
  const uint64_t ondisk_size = logical_to_ondisk_size(ret.oi.size, srd.shard);
  dup_error_cond(err, true, (smap_obj.size != ondisk_size), shard_info,
  ...
  ret.possible_auth = m_pg.get_is_nonprimary_shard(srd)
  ...
  const hobject_t& ho)
{
  ...
  this_chunk->m_current_obj = object_scrub_data_t{};
  ...
  dout(7) << fmt::format("{}: obj{} auth-avail:{}",
		__func__, ho, auth_res.is_auth_available) << dendl;
  ...
  ceph_assert(!this_chunk->m_current_obj.fix_digest);
  ...
  dout(7) << fmt::format("{}: obj{} opt-errs:{}",
		__func__, ho, opt_ers.has_value()) << dendl;
                                 ...
                                 stringstream& errstream)
{
  ...
  this_chunk->m_current_obj.cur_inconsistent.insert(object_errors.begin(),
  ...
  dout(15) << fmt::format(
                ...
                object_errors.size(),
                auth_list.size(),
                this_chunk->m_current_obj.cur_missing.size(),
                this_chunk->m_current_obj.cur_inconsistent.size())
           << dendl;


  if (!this_chunk->m_current_obj.cur_missing.empty()) {
    m_missing[ho] = this_chunk->m_current_obj.cur_missing;
  }
  if (!this_chunk->m_current_obj.cur_inconsistent.empty()) {
    m_inconsistent[ho] = this_chunk->m_current_obj.cur_inconsistent;
  }

  if (this_chunk->m_current_obj.fix_digest) {
  ...
  if (!this_chunk->m_current_obj.cur_inconsistent.empty() ||
      !this_chunk->m_current_obj.cur_missing.empty()) {

    this_chunk->authoritative[ho] = auth_list;

  } else if (!this_chunk->m_current_obj.fix_digest && m_is_replicated) {
  ...
  for (auto& [srd, smap] : this_chunk->received_maps) {
	...
	dout(10) << fmt::format(
      ...
      if (m_repair && m_is_replicated && (srd == auth_sel.auth_shard) &&
        ...
        this_chunk->m_current_obj.fix_digest = true;
        // Clear the error
      ...
      if (auth_sel.shard_map[srd].errors != 0) {
        ...
        this_chunk->m_current_obj.cur_inconsistent.insert(srd);
      ...
      this_chunk->m_current_obj.cur_missing.insert(srd);
