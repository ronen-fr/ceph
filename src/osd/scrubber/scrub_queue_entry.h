 ...
 * - do not require replicas to be reserved;
 * - cannot be aborted by 'noscrub'/'nodeep-scrub' flags;
 * - do not have random delays added to their target time;
 * - never have their target time delayed by a configuration change;
...
struct QueueInterface {
  ...
  virtual std::optional<Scrub::SchedEntry> dequeue_target(
      spg_t pgid,
      scrub_level_t level) = 0;
