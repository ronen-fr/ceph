...
void ReplicaActive::on_reserve_req(const ReplicaReserveReq& ev)
{
  ...
  if (reserved_by_my_primary) {
    ...
    clear_remote_reservation();  // clears the flag, too
  }
...
{
  DECLARE_LOCALS;  // 'scrbr' & 'pg_id' aliases
  dout(10) << "ReplicaActive::clear_remote_reservation()" << dendl;
...
{
  dout(10) << "-- state -->> ActiveAsReplicaReplicaActiveOp/ReplicaBuildingMap"
	   << dendl;
