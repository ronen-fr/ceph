...
ActiveScrubbing::~ActiveScrubbing()
{
  ...
  if (session.m_session_started_at != ScrubTimePoint{}) {
    ...
    scrbr->delay_next_scrub(Scrub::delay_cause_t::aborted);
 ...
 *   - move to the ProcessingReservation state
    ...
    bool async_request)
{
  DECLARE_LOCALS;  // 'scrbr' & 'pg_id' aliases
  dout(10) << fmt::format(
		  "ReplicaActive::on_reserve_request(): request:{} is async?{}",
		  ev, async_request)
	   << dendl;
  ...
  // should we deny the request if recovery is in progress?
  /// \todo reconsider
  if (false &&
      !scrbr->get_pg_cct()->_conf.get_val<bool>("osd_scrub_during_recovery") &&
      m_osds->is_recovery_active()) {

    reply = new MOSDScrubReserve(
	spg_t(pg_id.pgid, m_pg->get_primary().shard), ev.m_op->sent_epoch,
	MOSDScrubReserve::REJECT, m_pg->pg_whoami);
    dout(10) << fmt::format("{}: reserved? no (recover is active)", __func__)
	     << dendl;
    next_action = ReplicaReactCode::discard;

  } else if (async_request) {
    // the request is to be handled asynchronously
    svc.enqueue_remote_reservation(pg_id.pgid);
    next_action = ReplicaReactCode::goto_processing_reservation;
    ...
    if (granted) {
      ...
      reply = new MOSDScrubReserve(
	  spg_t(pg_id.pgid, m_pg->get_primary().shard), ev.m_op->sent_epoch,
	  MOSDScrubReserve::GRANT, m_pg->pg_whoami);
...
sc::result ReplicaUnreserved::react(const ReplicaReserveReq& ev)
{
  ...
  switch (context<ReplicaActive>().on_reserve_request(ev, false)) {
    case ReplicaReactCode::discard:
      return discard_event();
    case ReplicaReactCode::goto_processing_reservation:
      return transit<ProcessingReservation>();
...
// ---------------- ReplicaIdle/ProcessingReservation ---------------------------

ProcessingReservation::ProcessingReservation(my_context ctx)
    : my_base(ctx)
    , NamedSimply(
	  context<ScrubMachine>().m_scrbr,
	  "ReplicaActive/ReplicaIdle/ProcessingReservation")
{
  dout(10) << "-- state -->> ReplicaActive/ReplicaIdle/ProcessingReservation"
	   << dendl;
}


sc::result ProcessingReservation::react(const ReserverGranted&)
{
  DECLARE_LOCALS;  // 'scrbr' & 'pg_id' aliases
  dout(10) << "ProcessingReservation::react(const ReserverGranted&)" << dendl;
...
sc::result ProcessingReservation::react(const ReplicaRelease& ev)
{
  DECLARE_LOCALS;  // 'scrbr' & 'pg_id' aliases
  dout(10) << "ProcessingReservation::react(const ReplicaRelease&)" << dendl;
...
sc::result ReplicaReserved::react(const ReplicaReserveReq& ev)
{
  ...
  context<ReplicaActive>().clear_reservation_by_remote_primary();
  post_event(ev);
  return transit<ReplicaUnreserved>();
}
