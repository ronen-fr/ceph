...
tl::expected<std::set<snapid_t>, Scrub::mapper_req_t> SnapMapper::get_snaps(
  const hobject_t& oid) const
{
  ...
  if (r < 0) {
    dout(20) << __func__ << " " << oid << " got err " << r << dendl;
    return tl::unexpected(mapper_req_t{mapper_req_t::code_t::backend_error, r});
  }
  if (got.empty()) {
    dout(20) << __func__ << " " << oid << " got.empty()" << dendl;
    return tl::unexpected(
      mapper_req_t{mapper_req_t::code_t::not_found, -ENOENT});
  }
...
tl::expected<std::set<snapid_t>, Scrub::mapper_req_t>
				...
				std::set<snapid_t> snaps) const
{
  ...
  if (r < 0) {
    dout(10) << fmt::format("{}: backend error ({}) for clone {}",
	     ...
	     << dendl;
    return tl::unexpected(
      mapper_req_t{mapper_req_t::code_t::backend_error, r});
  }
...
SnapMapper::get_verified_snaps(const hobject_t& hoid) const
{
  ...
  if (!mapping_snaps) {
    ...
    // sense too).
    return tl::unexpected(mapper_req_t{mapper_req_t::code_t::inconsistent,
				       mapping_snaps.error().backend_error});
  }
