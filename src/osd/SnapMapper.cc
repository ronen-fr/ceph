...
//   set<string> keys;
//   auto got = backend.get_key(to_object_key(oid));
//   if (!got) {
//     // backend error
//     dout(20) << fmt::format("{}: {} got err {}", __func__, oid, got.error()) << dendl;
//     return got.error();
//   }
// 
//   if (out) {
...
// tl::expected<std::set<snapid_t>, Scrub::SnapMapReaderI::result_t> SnapMapper::get_snaps(
//   const hobject_t& oid) const
// {
//   ceph_assert(check(oid));
//   using SnapMapReaderI::result_t = Scrub::SnapMapReaderI::result_t;
// 
//   set<string> keys;
...
//     return tl::unexpected(SnapMapReaderI::result_t{SnapMapReaderI::result_t::code_t::backend_error, r});
//   }
//   if (got.empty()) {
//     dout(20) << __func__ << " " << oid << " got.empty()" << dendl;
//     return tl::unexpected(
//       SnapMapReaderI::result_t{SnapMapReaderI::result_t::code_t::not_found, -ENOENT});
//   }
// 
//   object_snaps out;
...
SnapMapper::get_snaps_common(const hobject_t &oid) const
{
  ...
  if (got.empty()) {
    dout(20) << __func__ << " " << oid << " got.empty()" << dendl;
    return tl::unexpected(
      result_t{code_t::not_found, -ENOENT});
  }
  ...
  try {
    ...
    return tl::unexpected(
      result_t{code_t::backend_error, -EIO});
  }
...
std::set<std::string> SnapMapper::expected_mapping_keys(
...
SnapMapper::get_mapping_entries(const hobject_t& clone,
...
tl::expected<std::set<snapid_t>, Scrub::SnapMapReaderI::result_t>
SnapMapper::get_verified_snaps(const hobject_t& hoid) const
{
  ...
  if (!mapping_snaps) {
    ...
    // sense too).
    return tl::unexpected(
      result_t{code_t::inconsistent, mapping_snaps.error().backend_error});
  }
