...
struct fmt::formatter<ceph::osd::scheduler::OpSchedulerItem> {
  ...
  template <typename FormatContext>
  auto format(const ceph::osd::scheduler::OpSchedulerItem& opsi, FormatContext& ctx)
  {
    // matching existing op_scheduler_item_t::operator<<() format
    const auto desc_qos =
	opsi.is_qos_item()
	    ? fmt::format(" class_id {}", opsi.get_scheduler_class())
	...
	opsi.get_ordering_token(), opsi.qitem->print(), desc_qos, qos_cost,
...
class PGScrubTryInitiating : public PGScrubItem {
 ...
 public:
  ...
  std::string print() const final {
    return fmt::format(
	"PGScrubTryInitiating(pgid={} level:{} token:{} env_conditions:{})",
