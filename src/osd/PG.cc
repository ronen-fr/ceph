...
void PG::handle_activate_map(PeeringCtx &rctx)
{
  ...
  // handle_activate_map() is called much too often - for
  // about every pg on every osd at ~1KHz - to unconditionally call
  // on_scrub_schedule_input_change() (which is performing quite a lot
  // of work). We should only dequeue,
  // recalculate & requeue if the pool opts have changed.
  if (pool.info.last_change == recovery_state.get_osdmap()->get_epoch()) {
