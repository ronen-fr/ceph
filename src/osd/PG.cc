    ...
    Scrub::OSDRestrictions osd_restrictions)
{
  ...
  if (!is_primary() || !is_active() || !is_clean()) {
    dout(10) << __func__ << ": cannot scrub (not a clean and active primary)"
	     << dendl;
    m_scrubber->delay_next_scrub(Scrub::delay_cause_t::pg_state);
  ...
  if (osd_restrictions.allow_requested_repair_only &&
    ...
    dout(10) << __func__
		...
		"requested for it"
	     << dendl;
    m_scrubber->delay_next_scrub(Scrub::delay_cause_t::scrub_params);
    return schedule_result_t::target_specific_failure;
  }
  ...
  if (!m_scrubber->reserve_local()) {
    ...
    m_scrubber->delay_next_scrub(Scrub::delay_cause_t::local_resources);
