...
and replica OSDs) to ensure that they are consistent. If the replicas all match,
a final semantic sweep takes place to ensure that all snapshot-related object
metadata is consistent.  Errors are reported in logs.

Scrubs initiated using the command format above are deemed 'high priority', and
are performed immediately. Such scrubs are not subject to the days-of-week or
