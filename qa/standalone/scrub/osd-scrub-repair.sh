...
function TEST_dual_store_replicated_cluster() {
    ...
    ceph_osd_args+="--osd_stats_update_period_scrubbing=2 --osd_op_queue=wpq "
    for osd in $(seq 0 1)
    do
...
EOF
    ...
    cp $dir/checkcsjson /tmp/temp_shallow_checkcsjson
    jq "$jqfilter" $dir/json | jq '.inconsistents' | python3 -c "$sortkeys" > /tmp/temp_shallow_actual
    jq "$jqfilter" $dir/json | jq '.inconsistents' | python3 -c "$sortkeys" > $dir/csjson
    multidiff $dir/checkcsjson $dir/csjson || test $getjson = "yes" || return 1
    if test $getjson = "yes"
    then
...
EOF
    ...
    cp $dir/checkcsjson /tmp/temp_deep_checkcsjson
    jq "$jqfilter" $dir/json | jq '.inconsistents' | python3 -c "$sortkeys" > /tmp/temp_deep_actual
    ...
    # now - perform a shallow scrub, and expect the shallow results go away, and the deep results remain

    bin/ceph tell osd.* osd_scrub_sleep 5
    bin/ceph tell osd.* scrubdebug $pg block
    bin/ceph pg scrub $pg
    sleep 5
    rados list-inconsistent-obj $pg > $dir/json_during_sh || return 1
    rados list-inconsistent-obj $pg > /tmp/json_during_sh
    rados list-inconsistent-obj $pg
    jq "$jqfilter" $dir/json_during_sh | jq '.inconsistents' | python3 -c "$sortkeys" > $dir/csjson_during_sh_srt
    cp $dir/csjson_during_sh_srt /tmp/csjson_during_sh_srt

    bin/ceph tell osd.* osd_scrub_sleep 0
    bin/ceph tell osd.* scrubdebug $pg unblock
    sleep 5
    ceph pg dump pgs

    rados list-inconsistent-obj $pg > $dir/json_after_sh || return 1
    rados list-inconsistent-obj $pg > /tmp/json_after_sh
    rados list-inconsistent-obj $pg
    jq "$jqfilter" $dir/json_after_sh | jq '.inconsistents' | python3 -c "$sortkeys" > $dir/json_after_sh_srt
    cp $dir/json_after_sh_srt /tmp/json_after_sh_srt
