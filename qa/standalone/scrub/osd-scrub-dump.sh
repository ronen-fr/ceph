...
#   optional entries:
#    ['creation_hook'] (empty) - a set of functions to be called on each object
#		after its creation. Call with the following parameters:
#		- the conf structure
#		- the name of the object
#		- the numerical index of the object
#		- RRR the PG active-set dict?
#  )
function corrupt_and_measure()
{
  ...
  # if a creation hook was defined - call it on each of the objects
  if [[ -n "${alargs['creation_hook']}" ]]; then
    for i in $(seq 1 $objects); do
      # Handle multiple functions in creation_hook (space-separated)
      for hook_func in ${alargs['creation_hook']}; do
        $hook_func "$dir" alargs "obj$i" $i "${obj_to_acting[obj$i]}" || return 1
      done
    done
  fi

  # --------------------------  step -: select objects to corrupt --------------------------
  #
...
function cre_hk_add_omap() {
  local dir="$1"
  local -n test_conf=$2
  local objname="$3"
  local objid="$4"
  local acting_set="$5"
  local poolname="${test_conf['pool_name']}"
  local entries=${test_conf['omap_entries_count']:-2}

  local object_is_selected=true
   if [[ -n "${alargs['omap_creation_select']}" ]]; then
    if ! "${alargs['omap_creation_select']}" "$objid"; then
      should_create_omap=false
    fi
  fi
  if [ "$object_is_selected" == "true" ]; then
    rados --pool "$poolname" setomapheader "$objname" "hdr-$objname" || return 1
    for ((i = 0; i < entries; i++)); do
      rados --pool "$poolname" setomapval "$objname" "key_$i" "value_$i" || return 1
    done
  fi
}
...
# remove one copy of an object
function rm_omap_kv_entry()
{
  local dir=$1
  local target_osd=$2
  local obj=$3
  ceph-objectstore-tool --data-path $dir/$target_osd "$obj" rm-omap key_0
  return 0
}
...
function creselect_omap_select_one() {
  local objid="$1"
  [[ "$objid" == 1 ]] && return 0
...
function TEST_time_measurements_basic_1()
{
  ...
  #corrupt_and_measure "$1" 3 8 16 24 24 || return 1
  local -A cls_conf=(
    ['osds_num']="3"
    ['pgs_in_pool']="8"
    ['pool_name']="test"
...
function TEST_omap_1()
{
  local -A cls_conf=(
    ...
    ['msg']="romap_1_rm"
    ['objects_per_pg']="2"
    ['modify_as_prim_cnt']="1"
    ['modify_as_repl_cnt']="1"
    ['manipulations']="rm_omap_kv_entry"
    ['omap_creation_select']="creselect_omap_select_one"
    ['omap_entries_count']="5"
    ['creation_hook']="cre_hk_add_omap"
  )
  corrupt_and_measure "$1" cls_conf || return 1
}
