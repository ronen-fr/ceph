...
function run() {
    ...
    CEPH_ARGS+="--osd_deep_scrub_update_digest_min_age=1 "
...
function corrupt_and_measure()
{
    ...
    (( extr_dbg >= 2 )) && echo "Object to PGID mapping:"
    ...
    declare -A pg_ac
    declare -A pg_po
    #build_pg_dicts_legacy "$dir" pg_pr pg_ac pg_po "-"
    ...
    for pg in "${!pg_pr[@]}"; do
        (( extr_dbg >= 1 )) && echo "saved-stamp $pg"
    ...
    sleep 2
    ceph pg dump pgs
    printf 'MSR NAUT %3d %3d %3d %3d/%3d %6d\n' "$OSDS" "$PGS" "$OBJS_PER_PG" "$CORRUPT_PRIMARY" "$CORRUPT_REPLICA" "$duration"
    for pg in "${!pg_pr[@]}"; do
    ...
    ceph tell osd.* config set osd_deep_scrub_update_digest_min_age 1
    ceph tell osd.* config set debug_osd 5/5
    start_time=$(date +%s%N)
    for pg in "${!pg_pr[@]}"; do
    ...
    for pg in "${!pg_pr[@]}"; do
      ...
      if [ "$incon_lines" -gt 5 ]; then
        echo "PG $pg has $incon_lines inconsistent objects ERRERRERRERRERRERRERRERRERRERRERRERRERRERRERRERR"
      fi
    done
    ceph pg ls-by-osd 0
    ceph pg ls-by-osd 1
...
{
  #corrupt_and_measure "$1" 3 4 16 0 8 || return 1
  # no problem observed: corrupt_and_measure "$1" 4 8 16 0 8 || return 1
  # no problem observed: corrupt_and_measure "$1" 4 8 32 8 8 || return 1
  # no problem observed: corrupt_and_measure "$1" 4 16 32 8 8 || return 1
  # corrupt_and_measure "$1" 4 16 64 8 8 || return 1
...
{
  #corrupt_and_measure "$1" 3 8 16 4 2 || return 1
  corrupt_and_measure "$1" 3 8 128 40 10 || return 1
}
...
function TEST_time_measurements_basic_3fast()
{
  #corrupt_and_measure "$1" 4 16 32 2 0 || return 1
  corrupt_and_measure "$1" 3 16 128 40 10 || return 1
}
