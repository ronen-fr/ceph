...
function blindly_set_pr_metadata {
    local pr_number="$1"
    local json_blob="$2"
    curl -H 'Authorization: Bearer ${github_token}' --silent --data-binary "$json_blob" "https://api.github.com/repos/ceph/ceph/issues/${pr_number}" >/dev/null 2>&1 || true
}
...
function cherry_pick_phase {
    ...
    remote_api_output=$(curl -H 'Authorization: Bearer ${github_token}' --silent "https://api.github.com/repos/ceph/ceph/pulls/${original_pr}")
...
function existing_pr_routine {
    ...
    remote_api_output="$(curl -H 'Authorization: Bearer ${github_token}' --silent "https://api.github.com/repos/ceph/ceph/pulls/${backport_pr_number}")"
...
function interactive_setup_routine {
    ...
    echo
    echo -n "Is the GitHub username (login) ronen-fr correct? "
    default_val="y"
    [ "$github_token" ] && echo "(default: ${default_val})"
    yes_or_no_answer="$(get_user_input "$default_val")"
    [ "$yes_or_no_answer" ] && yes_or_no_answer="${yes_or_no_answer:0:1}"
...
function milestone_number_from_remote_api {
    ...
    local milestones
    remote_api_output=$(curl -H 'Authorization: Bearer ${github_token}' --silent -X GET "https://api.github.com/repos/ceph/ceph/milestones")
...
function set_github_user_from_github_token {
    ...
    curl_opts="--silent -H 'Authorization: Bearer ${github_token}' https://api.github.com/user"
    [ "$quiet" ] || set -x
...
function vet_prs_for_milestone {
    ...
    remote_api_output="$(curl -H 'Authorization: Bearer ${github_token}' --silent --head "https://api.github.com/repos/ceph/ceph/pulls?base=${milestone_title}" | grep -E '^Link' || true)"
    ...
    for ((page=1; page<=pages_of_output; page++)) ; do
        ...
        remote_api_output="$(curl -H 'Authorization: Bearer ${github_token}' --silent -X GET "https://api.github.com/repos/ceph/ceph/pulls?base=${milestone_title}&page=${page}")"
...
function vet_remotes {
    ...
    if [ "$fork_remote" ] ; then
        ...
        if [ "$EXPLICIT_FORK" ] ; then
            ...
            info "(Could not find GitHub user ronen-fr's fork of ceph/ceph in \"git remote -v\")"
        fi
        setup_ok=""
    fi
}
...
if [ "$PR_PHASE" ] ; then
    ...
    remote_api_output=$(curl -H 'Authorization: Bearer ${github_token}' --silent --data-binary "{\"title\":\"${backport_pr_title}\",\"head\":\"${source_repo}:${local_branch}\",\"base\":\"${target_branch}\",\"body\":\"${desc}\"}" "https://api.github.com/repos/ceph/ceph/pulls")
